...
This file is a **template/guide**. Placeholders like `<...>` are expected here and must be replaced in real documents.
...
1. **context_dependencies**
  ...
  - In real docs: should be valid existing documents (or explicitly labeled example/historical)
...
5. Prefer repo-rooted paths for docs (e.g., `/docs/...`) for consistency
6. Verify referenced paths exist (or label as example/historical)
7. Update timestamps using the shipped helper when possible:
  - `/scripts/update-timestamp.ps1`